function [plcp_80211_framing_bits,plcp_framing_duration, output_xml] = func_phy_ieee80211(rate,mac_frame)
    ...
    plcp_80211_framing_bits = mac_frame * byte;
    plcp_framing_duration = 0;
    output_xml='<plcp-80211 /> \n';
    if(mac_frame >= plcp_mac_frame_min && mac_frame <= plcp_mac_frame_max)
        ...
        rate_preamble = dbpsk_transmitted_symbols_per_seconds * bits_per_symbol  / mb;
        ...
        rate_data = dbpsk_transmitted_symbols_per_seconds * bits_per_symbol  / mb;

        plcp_preamble_duration = ((plcp_preamble * byte) / (rate_preamble * mb)); %[seconds], see Gast, 2005,  chapter 12, The "Original" Direct Sequence PHY, Table 12-4. DS PHY parameters
        plcp_header_duration = ((plcp_header * byte) / (rate_preamble * mb)); %[seconds], see Gast, 2005,  chapter 12, The "Original" Direct Sequence PHY, Table 12-4. DS PHY parameters
        plcp_preamble_header_duration = plcp_preamble_duration + plcp_header_duration; %[seconds]
        plcp_data_duration = ((mac_frame * byte) / (rate_data * mb));%[seconds]
        ...
        plcp_80211_framing_bits = (mac_frame * byte) + (plcp_preamble * byte) + (plcp_header * byte);
    end
end
