function packet_stat(psize, bitrate, interval, packetfile,numbins,packet_stat_file)
% psize - Packetsize
...
allpackets( find( allpackets(:,7) > 60 ),7) = 0;                                 % remove wrong rssi

allpackets( find( allpackets(:,4) ~= psize ),3) = 0;   %all foreign, which don't match to the given Packetsize
allpackets( find( allpackets(:,5) ~= bitrate ),3) = 0; %all foreign, which don't match to the given Bitrate

own_packets = allpackets( find( allpackets(:,3) == 1 ),:);
own_packets_ok = own_packets(find( own_packets(:,2) == 0 ),:);
own_packets_crc = own_packets(find( own_packets(:,2) == 1 ),:);
own_packets_phy = own_packets(find( own_packets(:,2) == 2 ),:);

foreign_packets = allpackets( find( (allpackets(:,3) == 0) & (allpackets(:,2) == 0 ) ),:);       %foreignpacket: all packet are ok and foreign
...
for i=1:numbins
  ...
  bin_for_rssi = [ bin_own_rssi bin_own_rssi_ac];
...
clear bin_own_per_ac bin_own_rssi_ac bin_own_packet_count_ac bin_for_packet_count_ac bin_own_rssi_ac bin_for_own_rate_ac bin_for_medium_time_ac
