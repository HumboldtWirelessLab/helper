...
%    fitbackground:
%      - Fits background image to height or width
%                                     (value: 'height','widht')
%    removebackground:
...
function createmenu(hFigure,configuration)      
    ...
    uimenu(file, 'Label','Quit', 'Tag','uimenu_quit',  'Accelerator','Q', 'Callback',@quitgraphedit);
...
%     uimenu(edit, 'Label','Paste', 'Tag','uimenu_paste', 'Callback',@pastenodefcn, 'Accelerator','V', 'Enable','off');
...
%     uimenu(draw, 'Label','Layout', 'Tag','uimenu_layout', 'Callback',@newlayoutfornodes, 'Enable','on', 'Separator','on');
    ...
    uimenu(viewNodes, 'Label','View Nodes'' UserParams', 'Tag','uimenu_viewuserparamnode', 'Callback',{@viewtext,'userparam','node',findobj('Tag','contextmenu_viewparamnode')}, 'Checked','off');
    ...
    uimenu(viewEdges, 'Label','View Edges'' UserParams', 'Tag','uimenu_viewuserparamedge', 'Callback',{@viewtext,'userparam','edge',findobj('Tag','contextmenu_viewparamedge')}, 'Checked','on');
...
function createsliders(hFigure,visible)
    ...
    hSliderH = uicontrol(hFigure,...
        ...
        'Max',100,...
        'Visible',visible,...
        'Callback',{@sliderchanged,'horizontal'});
    hSliderV = uicontrol(hFigure,...
        'Style','slider',...
        'Enable','off',...
        'Visible',visible,...
        'Callback',{@sliderchanged,'vertical'});
...
%         [xLimGraph,yLimGraph] = getgraphlimitsovernodes(canvasData.nodes);
        ...
        xLimAxes = get(gca,'XLim');
        yLimAxes = get(gca,'YLim');
        xDelta = (xLimAxes(2)-xLimAxes(1))/2;
        yDelta = (yLimAxes(2)-yLimAxes(1))/2;
        pause(0.5);
        set(gca,'XLim',[xCenter - xDelta, xCenter + xDelta],...
...
function hAxes = createcanvas(hFigure,eventData,varargin)
    if nargin == 3, 
...
%             if sum(strcmp('curvature',fNames))
%                 structure{1} = setmaintocenter(structure{1},[structure{1}.x, structure{1}.y]);
%             end
        end
...
function makearrow(hEdge,visible)
    ...
    hLine1 = line('Parent',get(hEdge,'Parent'),'Color',get(hEdge,'Color'),'Tag','graphedit_arrow','Visible',visible);
    hLine2 = line('Parent',get(hEdge,'Parent'),'Color',get(hEdge,'Color'),'Tag','graphedit_arrow','Visible',visible);
...
function viewlibrarybrowser_command(hFigure,value)
    ...
    if strcmp(value,'off')
        ...
        if isempty(figureData.hlibrarybrowser),
            figureData.hlibrarybrowser = grapheditlibrarybrowser(...
...
function createtext(hObject,param)
    objectData = get(hObject,'UserData');
    ...
    if isempty(objectData.objectparams.TextParam)
        switch get(hObject,'Type')
            ...
            case 'line'
                ...
                %if deltaY == 0, deltaY = 0.001; end
                
                alpha = atan2(deltaY,deltaX);
                
                x1 = xData(index) + deltaX/3 + 10*sin(alpha);
                y1 = yData(index) + deltaY/3 + 10*cos(alpha);
                x2 = xData(index) + deltaX/3 + 20*sin(alpha);
                y2 = yData(index) + deltaY/3 + 20*cos(alpha);                
%                 if numel(xData) > 2
%                     x1 = round(xData(index-1) + (xData(index) - xData(index-1))/2);
%                     y1 = round(yData(index-1) + (yData(index) - yData(index-1))/2 - 10);
%                 else
%                     x1 = round(xData(1) + (xData(end) - xData(1))/2);
%                     y1 = round(yData(1) + (yData(end) - yData(1))/2 - 10);                   
%                 end
            otherwise
        end
...
%            copyobjectparams(hEdge,E{numEdge(j)});
            eps(i+j-1,:) = [hEdge, nodes(in(i)), nodes(fn(i))];
        end
    end
...
function exportcanvastopicture(hObject,eventData)
    ...
    extensions = {'.bmp','.jpg','.pbm','.pgm','.png','.ppm','.ras','.tiff'};
    ...
    if filename ~= 0
        ...
        if isempty(ext),
            ext = extensions{filterindex};
        end
        frame = getframe(gca);
        frame.cdata = frame.cdata(:,2:end,:);
        imwrite(frame.cdata,[pathname name ext],ext(2:length(ext)));      
        setactualpath('pictures',pathname);
    end
...
function viewtoolbar1(hMenu,varargin)
    if nargin == 2, 
        value = varargin{1};
    else
        value = switchmenuvalue(hMenu); 
...
function viewtoolbar2(hMenu,varargin)
    if nargin == 2, 
        value = varargin{1};
    else
        value = switchmenuvalue(hMenu); 
...
function viewtabs(hMenu,varargin)
    if nargin == 2, 
        value = varargin{1};
    else
        value = switchmenuvalue(hMenu); 
...
function viewsliders(hMenu,varargin)
    if nargin == 2, 
        value = varargin{1};
    else
        value = switchmenuvalue(hMenu); 
...
function tab = getactualtab
    figureData = get(gcf,'UserData');
    children = get(figureData.htabs,'Children');
    axesbarData = get(figureData.htabs,'UserData');
    tab = find(axesbarData.flags == children(1));
