...
%   $Revision: 1820 $  $Date: 2007-09-21 13:58:58 +0200 (pá, 21 IX 2007) $
...
while(paramCounter<=nargin)
    if(~isa(varargin{paramCounter},'char'))
        error(sprintf('Input argument %d must be a string of parameter name.',paramCounter));
    end;
    if((paramCounter+1)>nargin)
        error(sprintf('Value of input parameter "%s" was not specified.',varargin{paramCounter}));
    end;
    ...
    switch(varargin{paramCounter})
        case 'maxIter'
            if(~isa(value,'numeric'))
                error(sprintf('Invalid value for options parameter ''maxIter'': must be a real positive number.'));
            end;
            schoptions.maxIter=round(value);

        case 'verbose'
            if(~isa(value,'numeric'))
                error(sprintf('Invalid value for options parameter ''verbose'': must be 0,1 or 2.'));
            end;
        ...
        case 'logfile'
            if(~isa(value,'numeric'))
                error(sprintf('Invalid value for options parameter ''logfile'': must be 0 or 1.'));
            end;
            if(value~=0 & value~=1)
                error(sprintf('Invalid value for options parameter ''logfile'': must be 0 or 1.'));
            end;
        ...
        case 'logfileName'
            if(~isa(value,'char'))
                error(sprintf('Invalid value for options parameter ''logfileName'': must be a real positive number.'));
            end;
            schoptions.logfileName=varargin{paramCounter+1};

        case 'strategy'
            if ~((isa(value,'char'))|(isa(value,'function_handle')))
                error(sprintf('Invalid value for options parameter ''strategy'': must be a string or function handle.'));
            end;
            schoptions.strategy=value;

        case 'ilpSolver'
            if(~isa(value,'char'))
                error(sprintf('Invalid value for options parameter ''ilpSolver'': must be a string ''glpk'',''lp_solve'',''cplex'',''cplex2'' or ''external''.'));
            end;
        ...
        case 'extIlinprog'
            if(~isa(value,'function_handle'))
                error(sprintf('Invalid value for options parameter ''extIlinprog'': must be a function handle.'));
            end;
            schoptions.extIlinprog=varargin{paramCounter+1};

        case 'miqpSolver'
            if(~isa(value,'char'))
                error(sprintf('Invalid value for options parameter ''miqpSolver'': must be a string ''miqp'',''cplex'' or ''external''.'));
            end;
        ...
        case 'extIquadprog'
            if(~isa(value,'function_handle'))
                error(sprintf('Invalid value for options parameter ''extIquadprog'': must be a function handle.'));
            end;
            schoptions.extIquadprog=varargin{paramCounter+1};

        case 'solverVerbosity'
            if(~isa(value,'numeric'))
                error(sprintf('Invalid value for options parameter ''solverVerbosity'': must be 0,1 or 2.'));
            end;
            if(value~=0 & value~=1 & value~=2)
                error(sprintf('Invalid value for options parameter ''solverVerbosity'': must be 0,1 or 2.'));
            end;
        ...
        case 'solverTiLim'
            if(~isa(value,'numeric'))
                error(sprintf('Invalid value for options parameter ''solverTiLim'': must be a real positive number.'));
            end;
            schoptions.solverTiLim=varargin{paramCounter+1};

        case 'cycSchMethod'
            if(~isa(value,'char'))
                error(sprintf('Invalid value for options parameter ''cycSchMethod'': must be a string ''integer'' or ''binary''.'));
            end;
        ...
        case 'varElim'
            if(~isa(value,'numeric'))
                error(sprintf('Invalid value for options parameter ''varElim'': must be 0 or 1.'));
            end;
            if(value~=0 & value~=1)
                error(sprintf('Invalid value for options parameter ''varElim'': must be 0 or 1.'));
            end;
        ...
        case 'varElimILPSolver'
            if(~isa(value,'char'))
                error(sprintf('Invalid value for options parameter ''varElimILPSolver'': must be a string ''glpk'',''lp_solve'',''cplex'',''cplex2'' or ''external''.'));
            end;
            availableSolvers={'glpk','lp_solve','cplex','cplex2','external'};
            if(~ismember({value},availableSolvers))
                error(sprintf('Invalid value for options parameter ''varElimILPSolver'': must be a string ''glpk'',''lp_solve'',''cplex'',''cplex2'' or ''external''.'));
        ...
        case 'secondaryObjective'
            if(~isa(value,'numeric'))
                error(sprintf('Invalid value for options parameter ''secondaryObjective'': must be 0 or 1.'));
            end;
            if(value~=0 & value~=1)
                error(sprintf('Invalid value for options parameter ''secondaryObjective'': must be 0 or 1.'));
            end;
        ...
        case 'qmax'
            if(~isa(value,'numeric'))
                error(sprintf('Invalid value for options parameter ''qmax'': must be greather or equal to 0 or [].'));
            end;
            if(~isempty(value))
                if(value<0)
                    error(sprintf('Invalid value for options parameter ''qmax'': must be greather or equal to 0 or [].'));
        ...
        case 'spntlMethod'
            if(~isa(value,'char'))
                error(sprintf('Invalid value for options parameter ''spntlMethod'': must be a string ''BaB'', ''BruckerBaB'' or ''ILP''.'));
            end;
        ...
        otherwise
            error(sprintf('Unrecognized parameter name ''%s''.',varargin{paramCounter+1}));
    end;
