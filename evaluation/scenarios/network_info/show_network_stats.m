...
if (show_mcg)
    ...
    for succ_i=1:size(succv,2)
        ...
        if (max(max(mygr)) > 0)
           ...
           s.connected_graphs.connected_graph{succ_i}.clusters{ii}.Attributes.bitrate = bitrate;
           s.connected_graphs.connected_graph{succ_i}.clusters{ii}.Attributes.number = max(C);
           for kk=1:max(C)
              bdata(jj,kk) = size(find(C==kk),2);
              cluster_nodes = nodelst(find(C==kk));
              s.connected_graphs.connected_graph{succ_i}.clusters{ii}.cluster{kk}.Attributes.id = kk;
              s.connected_graphs.connected_graph{succ_i}.clusters{ii}.cluster{kk}.Attributes.size = size(find(C==kk),2);
              str_cl_nodes = '';
              for pp=1:size(cluster_nodes,2)
                s.connected_graphs.connected_graph{succ_i}.clusters{ii}.cluster{kk}.node{pp}.Text = cluster_nodes{pp};
                str_cl_nodes = [str_cl_nodes, ',', int2str(cluster_nodes{pp})];
              end
              disp(['SR=', int2str(succv(succ_i)), ',C=', int2str(kk), ',N=[', str_cl_nodes, ']']);
           end
           jj = jj + 1;
        end
