    ...
    switch lower(varargin{i})
        case 'maxtime'
            if ischar(varargin{i+1})
                max_time=str2num(varargin{i+1});
            else
                max_time=varargin{i+1};
            end
        case 'proc'
            if ischar(varargin{i+1})
                grouped=str2num(varargin{i+1});
            else
                grouped=varargin{i+1};
            end
        case 'color'
            if ischar(varargin{i+1})
                setcol=str2num(varargin{i+1});
            else
                setcol=varargin{i+1};
            end
        case 'asap'
            if ischar(varargin{i+1})
                asapdraw=str2num(varargin{i+1});
            else
                asapdraw=varargin{i+1};
            end
        case 'axis'
            if ischar(varargin{i+1})
                axis_user=str2num(varargin{i+1});
            else
                axis_user=varargin{i+1};
            end
        case 'prec'
            if ischar(varargin{i+1})
                prec_draw=str2num(varargin{i+1});
            else
                prec_draw=varargin{i+1};
            end
        case 'period'
            if ischar(varargin{i+1})
                period_number=str2num(varargin{i+1});
            else
        ...
        case 'weight'
            if ischar(varargin{i+1})
                weight_sort=str2num(varargin{i+1});
            else
                weight_sort=varargin{i+1};
            end
        case 'reverse'
            if ischar(varargin{i+1})
                reverse_order=str2num(varargin{i+1});
            else
                reverse_order=varargin{i+1};
            end
        case 'axname'
            if ischar(varargin{i+1})
                axname=str2num(varargin{i+1});
            else
                axname=varargin{i+1};
            end
        case 'textins'
            if ischar(varargin{i+1})
                textins=str2num(varargin{i+1});
            else
                textins=varargin{i+1};
            end
        case 'psgantt'
            if ischar(varargin{i+1})
                psgantt=str2num(varargin{i+1});
            else
...
for i=1:length(T.tasks)
    ...
    if grouped
        if prod(size(task_struct.schProcessor))
            mov_top{I} = task_struct.schProcessor-1;
        else
            if prod(size(task_struct.Processor))
...
for i=1:length(T.tasks)
    ...
    if isa(task, 'ptask')
        plot(task,'movtop',(mov_top_max+mov_top_max_rotation*mov_top{I})*distance,'texton',0,'maxtime', max_time,'textin',grouped,'asap',asapdraw,'period',0,'textins',textins);
    else
        plot(task,'movtop',(mov_top_max+mov_top_max_rotation*mov_top{I})*distance,'texton',0,'textin',grouped,'asap',asapdraw,'period',0,'textins',textins);
    end
end
...
if prec_draw
    linked_from = zeros(1,count(T));
    linked_to = zeros(1,count(T));
...
% ----------------------------------------------------------
function psganttviewclassical(hObject,eventData)
...
% ----------------------------------------------------------
function psganttpreview(hObject,eventData)
UserData = get(findobj('Tag','psganttmenu'),'UserData');
position = get(UserData.hFigure,'Position');
...
% ----------------------------------------------------------
function psganttsaveas(hObject,eventData)
try
...
% ----------------------------------------------------------
function psganttaddcss(hObject,eventData)
...
% ----------------------------------------------------------
function psgantteditcss(hObject,eventData)
UserData = get(findobj('Tag','psganttmenu'),'UserData');
...
% ----------------------------------------------------------
function psgantteditxml(hObject,eventData)
UserData = get(findobj('Tag','psganttmenu'),'UserData');
