function prob = problem(notation)
...
%         Cmax, sumCj, sumwCj, Lmax, sumDj, sumUj
%
...
if any(strcmp(notation,specialNotationDefinition))
    ...
    if na >= 1
        ...
        while length(betha_notation)
            ...
            if (~sum(strcmpi(betha_notation,{'pmtn','rj','~dj','prec','in-tree'}))) & (isempty(regexpi(betha_notation,'^pj=\d+$')))
