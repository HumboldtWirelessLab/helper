...
cat $RESULTDIR/measurement.log | grep "link from" | grep -v 'metric="9999"' | sed 's#"# #g' | awk '{print $3" "$5}' | sort -u > $EVALUATIONSDIR/links.all
cat $RESULTDIR/measurement.log | grep "link from" | grep -v 'metric="9999"' | sed 's#"# #g' | awk '{print $3" "$5" "$7}' | sort -u > $EVALUATIONSDIR/linksmetric.all
