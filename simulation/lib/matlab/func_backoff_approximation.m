...
function [matrix_packet_loss_neighbours_backoff_windows_approximation] = func_backoff_approximation(vector_packet_loss,vector_neighbours)
    ...
    for i =1:1:size(vector_packet_loss,2)
        ...
        for j=1:1:size(vector_neighbours,2)
             denominator = (1-(1-vector_packet_loss(1,i))^(1/(vector_neighbours(1,j))));
            if (denominator > 0)
                %matrix_w_approximate_all(i,counter) = 1/(1 - (1-pc))^(1/j);
                %if ((~isempty(find(vector_neighbours == j, 1)))) %|| counter_neighbours_add == j) && j > 1) 
                    %if (matrix_w_approximate_all(i,counter) >= 0)
                        matrix_packet_loss_neighbours_backoff_windows_approximation(i,j) = ceil(1/denominator);%ceil(matrix_w_approximate_all(i,counter)); %there are only int slots, for that reason round up
