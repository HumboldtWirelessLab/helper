...
function [matrix_packet_loss_neighbours_backoff_windows_approximation] = func_backoff_approximation_2(vector_packet_loss,vector_neighbours)
    ...
    for i =1:1:size(vector_packet_loss,2)
        ...
        for j=number_of_neighbours_sizes_min:1:number_of_neighbours_max - 1
            if (pc > 0)
                matrix_w_approximate_all(i,counter) = 1/(1 - (1-pc)^(1/(j + 1)));
