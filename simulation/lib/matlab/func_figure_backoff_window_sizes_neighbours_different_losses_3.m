function [ handler_figure ] = func_figure_backoff_window_sizes_neighbours_different_losses_3(figure_number,w_approximate,vector_packet_loss,vector_backoff_window_sizes_standard)
...
hold all
%hplots = zeros(size(w_approximate,1),1);
%for t = 1:1:size(w_approximate,1)
%    no_neighbours = 1:1:w_approximate(t,1);
...
%plot(v_neighbours,w_approximate,'-x')
   hplots = plot(w_approximate,'-x','LineWidth',2);
...
grid on
neighbours_max_2 = max(size(w_approximate,1)) + 10;
...
% end
 ...
 %end
   ...
   max_y = max(size(w_approximate,2))+1;
x=zeros(1,max_y);
