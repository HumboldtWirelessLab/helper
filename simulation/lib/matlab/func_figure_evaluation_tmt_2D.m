function [handler_figure] = func_figure_evaluation_tmt_2D(figure_number,vector_neighbours, matrix_tmt_backoff_2D,text_label_x,text_label_y,ticks_y_step_size,ticks_y_max,text_title,legend_on,vector_legend1,text_legend_title1,vector_legend2,text_legend_title2)%,vector_backoff_window_sizes_standard) 
    ...
    hcolor_counter = 1;
    linestyle_counter = 1;
    vector_result = zeros(1,size(matrix_tmt_backoff_2D,3));
    vector_hlegend_style_marker = zeros(1,size(vector_legend1,2));
    vector_hlegend_style_color = zeros(1,size(vector_legend2,2));
    for i = 1:1:size(vector_legend1,2) %data_rates 
        for j = 1:1:size(vector_legend2,2) %msdu_sizes         
            vector_result(1,:) = matrix_tmt_backoff_2D(i,j,:);
            vector_hlegend_style_color(1,i) = plot(vector_neighbours,vector_result,LineStyle_lines{1,2},'Color',ColorSet(hcolor_counter,1:3),'LineWidth',2);
            vector_hlegend_style_marker(1,j) = plot(vector_neighbours,vector_result,LineStyle_marker{1,linestyle_counter},'LineWidth',2);
            plot(vector_neighbours,vector_result,sprintf('%s%s',LineStyle_lines{1,2},LineStyle_marker{1,linestyle_counter}),'Color',ColorSet(hcolor_counter,1:3),'LineWidth',2);
    ...
    handler_legend2 = legend(vector_hlegend_style_marker,legend_txt2,'Location','North');
