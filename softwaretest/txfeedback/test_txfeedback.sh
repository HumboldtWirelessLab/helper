...
RESULT=`RUNMODE=REBOOT CONFIGFILE=$DIR/monitortest.mes.tmp MARKER=$REVISION STATUSFD=5 ID=txfeedback TIME=$TIME $DIR/../../measurement/bin/run_single_measurement.sh 5>&1 1>> $LOGDIR/measurement.log 2>&1`
...
if [ "$RESULT" = "error" ]; then
	...
	PACKETCOUNT=`cat $LOGDIR/sender.log | grep "^Fee" | wc -l`
