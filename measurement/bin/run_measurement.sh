...
if [ "x$TESTONLY" = "x" ]; then
  ...
  echo "(cd $FINALRESULTDIR; WANTNODELIST=$WANTNODELIST FINALRESULTDIR=$FINALRESULTDIR REMOTEDUMP=$REMOTEDUMP LOCALPROCESS=$LOCALPROCESS CLICKMODE=$CLICKMODE CONFIGFILE=$NODETABLE STATUSFD=5 TIME=$TIME ID=$NAME RUNMODE=$RUNMODE DISABLE_WIRELESS_BACKBONE=$DISABLE_WIRELESS_BACKBONE $DIR/run_single_measurement.sh 5>&1 6>&2 1>> $LOGDIR/$LOGFILE 2>&1)" >> $FINALRESULTDIR/run_again.sh

  RESULT=`(cd $FINALRESULTDIR; WANTNODELIST=$WANTNODELIST FINALRESULTDIR=$FINALRESULTDIR REMOTEDUMP=$REMOTEDUMP LOCALPROCESS=$LOCALPROCESS CLICKMODE=$CLICKMODE CONFIGFILE=$NODETABLE STATUSFD=5 TIME=$TIME ID=$NAME RUNMODE=$RUNMODE DISABLE_WIRELESS_BACKBONE=$DISABLE_WIRELESS_BACKBONE $DIR/run_single_measurement.sh 5>&1 6>&2 1>> $LOGDIR/$LOGFILE 2>&1)`
