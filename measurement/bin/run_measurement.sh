...
if [ "x$DEV" = "x1" ]; then
  RESULT=`(cd $FINALRESULTDIR; WANTNODELIST=$WANTNODELIST FINALRESULTDIR=$FINALRESULTDIR REMOTEDUMP=$REMOTEDUMP LOCALPROCESS=$LOCALPROCESS CLICKMODE=$CLICKMODE CONFIGFILE=$NODETABLE MARKER=$NAME STATUSFD=5 TIME=$TIME ID=$NAME RUNMODE=$RUNMODE $DIR/run_single_measurement_dev.sh 5>&1 6>&2 1>> $LOGDIR/$LOGFILE 2>&1)`
else
