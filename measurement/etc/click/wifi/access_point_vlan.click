elementclass ACCESS_POINT_VLAN { DEVICE $device, ETHERADDRESS $etheraddress, SSID $ssid, CHANNEL $channel,
...
//  -> Print("assocReq")
    -> BRN2AssocResponder(DEBUG 0, DEVICE $device, WIRELESS_INFO winfo,
                          RT rates, ASSOCLIST assoclist, WIRELESSINFOLIST wil, VLANTABLE vlt, RESPONSE_DELAY 0 )
    -> [0]output;
...
//  -> Print("assocResp")
    ...
    mgt_cl[2]
//  -> Print("probereq")
    -> BRN2BeaconSource( WIRELESS_INFO winfo, WIRELESSINFOLIST wil, RT rates,ACTIVE 1)
//  -> Print("proberesp")
    -> [0]output;
